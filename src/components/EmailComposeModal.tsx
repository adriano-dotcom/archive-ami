...
import { X, Send, Loader2, ChevronDown, Sparkles, User, Building2, MapPin, Package, FileText, TrendingUp, Pencil, Eye, AlertTriangle, Calendar, Receipt } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {/* Collection Context Card - Modo Cobrança */}
          {collectionContext && (
            <div className="bg-gradient-to-br from-amber-900/30 to-orange-900/20 border border-amber-500/30 rounded-xl p-4 space-y-3">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-amber-400" />
                <span className="font-semibold text-white">Email de Cobrança</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="bg-slate-800/50 rounded-lg p-2.5">
                  ...
                  <p className="text-amber-400 font-bold">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(collectionContext.totalOverdue)}
                  </p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
                  ...
                  <p className="text-white font-medium">{collectionContext.installmentsCount} pendente(s)</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
                  ...
                  <p className="text-red-400 font-medium">{collectionContext.maxDaysOverdue} dias</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-2.5">
                  <div className="flex items-center gap-1.5 text-slate-400 text-xs mb-1">
                  ...
                  <p className="text-white font-medium truncate" title={collectionContext.companyName}>
                    {collectionContext.companyName || 'Não informada'}
                  </p>
                </div>
              </div>
              ...
              >
                {generatingAI ? (
                  <>
                    ...
                    Gerando Email de Cobrança...
                  </>
                ) : (
                  <>
                    <Sparkles className="w-4 h-4 mr-2" />
                    Gerar Email de Cobrança
                  </>
                )}
              </Button>
            </div>
