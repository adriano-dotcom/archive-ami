...
-- Fix views to use SECURITY INVOKER (default in PG15+, explicit for clarity)
CREATE OR REPLACE VIEW public.orbe_support_tickets_v
...
CREATE OR REPLACE VIEW public.orbe_support_daily_metrics_v
WITH (security_invoker = true)
AS
WITH today AS (
...
SELECT
  ...
  (
    SELECT jsonb_object_agg(
      CASE s WHEN 'nina' THEN 'open_nina' WHEN 'human' THEN 'open_human' WHEN 'paused' THEN 'pending' WHEN 'closed' THEN 'closed' ELSE s::text END,
      cnt
    )
    FROM (SELECT status::text AS s, COUNT(*)::int AS cnt FROM conversations GROUP BY status) sub
  ) AS by_status_json,
  (
    SELECT jsonb_object_agg(COALESCE(assigned_user_name, 'Nina (IA)'), cnt)
    FROM (SELECT assigned_user_name, COUNT(*)::int AS cnt FROM conversations WHERE status IN ('nina', 'human', 'paused') GROUP BY assigned_user_name) sub
  ) AS by_assigned_to_json
FROM today t;

CREATE OR REPLACE VIEW public.orbe_support_weekly_metrics_v
WITH (security_invoker = true)
AS
WITH week_range AS (
  SELECT
...
sla_check AS (
  ...
  WHERE c.status IN ('nina', 'human', 'paused') AND c.last_message_at < now() - interval '24 hours'
),
waiting_customer AS (
  SELECT c.id FROM conversations c
  WHERE c.status IN ('nina', 'human', 'paused')
    AND (SELECT m.from_type FROM messages m WHERE m.conversation_id = c.id ORDER BY m.sent_at DESC LIMIT 1) IN ('nina', 'human')
)
SELECT
  ...
  (
    ...
    FROM (SELECT status::text AS s, COUNT(*)::int AS cnt FROM conversations
      WHERE (created_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN w.week_start AND w.week_end OR status IN ('nina', 'human', 'paused')
      GROUP BY status) sub
  ) AS by_status_json,
  (
    SELECT jsonb_object_agg(COALESCE(assigned_user_name, 'Nina (IA)'), cnt)
